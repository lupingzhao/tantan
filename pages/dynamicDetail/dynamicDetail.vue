<template>
	<view class="p-10">
		...
		<view class="m-t-10" id="comment">
			评论
		</view>
		<!-- 评论详情 -->
		<view class="m-t-10" >
			<view class="" @click="commet">
				<view v-for="item in 9" :key="item.id" class="mb-10">
		...
		<!-- 发表评论 -->
		<view class="flex a-i-fc m-t-10 inputComment p-lr-10" v-show="inputComment" 
		:style="{bottom:keyHeight}">
			<input type="text" @blur="inputComment=!inputComment" :value="comment" class="input_comment" placeholder="评论" confirm-type="发送"
				@confirm="putComment" />
